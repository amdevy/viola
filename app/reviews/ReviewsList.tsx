...
export default function ReviewsList({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success("Дякуємо! Ваш відгук буде опублікований після модерації.");
      setShowForm(false);
      setFormData({ name: "", rating: 5, text: "" });
    } catch {
      toast.error("Помилка надсилання відгуку. Спробуйте ще раз.");
    }
    setSubmitting(false);
  };
        ...
        {reviews.length > 0 ? (
          <div className="flex items-center gap-4">
            <div className="text-center">
              ...
              <p className="text-xs text-[#6B6B6B] mt-1">{reviews.length} відгуків</p>
            </div>
          </div>
        ) : (
          <p className="text-[#6B6B6B]">Відгуків поки немає. Будьте першими!</p>
        )}
        <Button onClick={() => setShowForm(true)}>Залишити відгук</Button>
      </div>
